...
`data/live_zone_telemetry.json` is the loopable 1-second idle monitoring stream for zone cards and Prime Mover status before scenario evidence is active.
...
`data/scenario_zone_telemetry/{scenario}.json` contains dense 1-second dynamic zone telemetry aligned to each selected scenario. Static zone registry/context remains in `data/zones.json`.
